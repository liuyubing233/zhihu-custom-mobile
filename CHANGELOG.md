...
- 🐛 修复target.id精度丢失导致接口参数异常获取接口数据失败。
...
- 🆕 添加顶部搜索框配置，默认显示。
- 🆕 修改`展开更多`按钮类名，跟知乎原生保持一致（用来解决部分插件或者浏览器会自动展开更多问题？）

## 2.8.0

`2024-07-04`

- 🆕 添加方法实施监听并关闭打开 APP 弹窗。
- 🐛 解决数学公式消失的问题。
- 🐛 解决回答内容被覆盖导致展开更多回答、评论消失，点赞无法点击的问题。
- 🐛 解决部分情况下展开更多点击没反应的问题。
- 🎨 修改回答下视频的结构，修复自动播放的问题。
- 🎨 修改覆盖重载的逻辑。
...
- 🆕 兼容按照时间顺序查看回答内容的页面逻辑。
- 👽 隐藏部分浏览器下存在的底部跳转 app 的评论赞同栏。

## 2.6.1

`2024-05-14`

- 🐛 修复只有一个回答时无限请求和唯一回答消失的问题。
