...
(() => {
  ...
  var openCommentLoading = () => dom(`#${ID_CTZ_COMMENT} ${QUERY_LOADING}`).style.display = "block";
  var hideCommentLoading = () => dom(`#${ID_CTZ_COMMENT} ${QUERY_LOADING}`).style.display = "none";
  var openCommentEnd = () => dom(`#${ID_CTZ_COMMENT} ${QUERY_END}`).style.display = "block";
  var hideCommentEnd = () => dom(`#${ID_CTZ_COMMENT} ${QUERY_END}`).style.display = "none";
  var createCommentHTML = (data) => data.map((i2) => createCommentHTMLItem(i2)).join("");
  var createCommentHTMLItem = (item, isChild = false) => {
    const {
      author,
      id,
      author_tag,
...
<div data-id="${id}">
  ...
  ${child_comments.map((i2) => createCommentHTMLItem(i2, true)).join("")}
  ${child_comment_count > child_comments.length ? `<button class="ctz-comment-button" name="comment-more" data-next-offset="${child_comment_next_offset}">查看全部 ${child_comment_count} 条回复 ➤</button>` : ""}
</div>`;
  };
